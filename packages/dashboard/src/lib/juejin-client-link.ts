...
const DEFAULT_JUEJIN_HOME = 'https://juejin.cn/';
const DEFAULT_JUEJIN_ONLINE_PAGE = 'https://juejin.cn/aiusage/dashboard';

function resolveConfiguredHttpUrl(
...
/** Page where an unauthenticated user can complete Juejin login. */
export const JUEJIN_HOME = resolveConfiguredHttpUrl(
  import.meta.env.VITE_AUTH_LOGIN_URL,
  DEFAULT_JUEJIN_HOME,
);
/** Land on /dashboard directly — avoids `/` → `/dashboard` redirect dropping Query. */
export const JUEJIN_ONLINE_PAGE = resolveConfiguredHttpUrl(
  import.meta.env.VITE_ONLINE_PAGE_URL,
  DEFAULT_JUEJIN_ONLINE_PAGE,
);
...
function tudSessionUrl(): string {
  ...
  return base ? `${base}/functions/tud-session` : '/api/functions/tud-session';
}
...
/**
 * Open Juejin's current login surface in another tab. Juejin renders login as
 * an in-page modal rather than a stable dedicated route, so the dashboard
 * re-checks tud-session when this tab regains focus.
 */
export function openJuejinAccountLogin(): void {
  const opened = window.open(JUEJIN_HOME, '_blank');
  if (opened) {
    opened.opener = null;
    return;
  }
  window.location.assign(JUEJIN_HOME);
}
